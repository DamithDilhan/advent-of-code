    ...
    for other in sc_dict.values():
        for axis in (0, 1, 2):
            for sign in (-1, 1):
                ...
                if n >= 12:
                    x_coords[other.id] = AxisInfo(axis=axis, sign=sign, diff=x_diff,)
    ...
    for scanner in x_coords:
        ...
        for axis in (0, 1, 2):
            for sign in (-1, 1):
                ...
                if n >= 12:
                    y_coords[other.id] = AxisInfo(axis=axis, sign=sign, diff=y_diff,)
                ((z_diff, n),) = d_z.most_common(1)
                if n >= 12:
                    z_coords[other.id] = AxisInfo(axis=axis, sign=sign, diff=z_diff,)
...
if __name__ == "__main__":
    ...
    for i, (x0, y0, z0) in enumerate(pos):
        for x1, y1, z1 in pos[i + 1 :]:
            maximum_dist = max(abs(x1 - x0) + abs(y1 - y0) + abs(z1 - z0), maximum_dist)
